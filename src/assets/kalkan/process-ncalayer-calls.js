...
function clearArrXML() {
  console.log('clearArrXML');
  arrXML = [];
}

function addArrXML(xml) {
  console.log('addArrXML', xml);
  arrXML.push(xml);
}

function getArrXML() {
  console.log('arrXML in proc: ', arrXML);
  return arrXML;
}

function getResXML() {
  // console.log('resXML in proc: ', resXML);
  return resXML;
}

function signArrXMLCall() {
  console.log('processing signArrXMLCall');
  var xmlsToSign = new Array();
  arrXML.forEach(xml => {
    if (xml != null && xml != "") {
      xmlsToSign.push(xml);
    }
  ...
  blockScreen();
  signXmls("PKCS12", "SIGNATURE", xmlsToSign, "signArrXMLBack");
}

function signArrXMLCall2() {
  ...
  if (webSocket === null || webSocket.readyState === 3 || webSocket.readyState === 2) {
    callNCALayer('signXmlCall2');
  } else {
    signXmlCall2();
  }
}

function signXmlCall2() {
  var xmlToSign = '<?xml version="1.0" encoding="utf-8"?><test>1234567</test>';
  var selectedStorage = "PKCS12";
  //blockScreen();
  signXml2(selectedStorage, "AUTHENTICATION", xmlToSign, "signXmlBack2");
}

function signXmlBack2(result) {
  // unblockScreen();
  if (result['code'] === "500") {
    alert(result['message']);
  } else if (result['code'] === "200") {
    resXML = result['responseObject'];
    // console.log(resXML);
    window['callAngularSignXMLFunction'](resXML);
  }
}

function signArrXMLBack(result) {
  console.log('processing signArrXMLBack');
  unblockScreen();
  if (result['code'] === "500") {
    alert(result['message']);
  } else if (result['code'] === "200") {
    ...
    window['callAngularSignXMLFunction'](arrXML[0]); //, arrXML[1], arrXML[2], arrXML[3], arrXML[4], arrXML[5], arrXML[6]);
  }
}
...
function signXmlsCall() {
  clearArrXML();
  addArrXML($("#xmlToSign1").val());
  addArrXML($("#xmlToSign2").val());
  var selectedStorage = $('#storageSelect').val();
  blockScreen();
  signXmls(selectedStorage, "SIGNATURE", arrXML, "signXmlsBack");
}
...
function signXmlsBack(result) {
  ...
  if (result['code'] === "500") {
    ...
    arrXML[0] = res[0];
    arrXML[1] = res[1];
    $("#signedXml1").val(arrXML[0]);
    $("#signedXml2").val(arrXML[1]);
  }
}
